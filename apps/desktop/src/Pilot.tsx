...
export function Pilot(){
 ...
 return <>
  ...
  {!room?<div className="two-columns pilot-entry">
   ...
   {room.role==='viewer'&&room.state!=='ended'&&<><div className="viewer"><video ref={video} autoPlay muted playsInline controls onPlaying={()=>{setReceived(true);setVideoState('Canlı görüntü')}}/>{!received&&<div className="viewer-placeholder"><LoaderCircle size={25} className="spin"/><span>{room.state==='awaiting_consent'?'Karşı bilgisayardaki kullanıcının onayı bekleniyor':videoState}</span></div>}</div><p className="small-description">{videoState} · Görüntü kaydedilmez. Pilot yalnızca izleme destekler.</p></>}
