...
const labels:Record<string,string>={waiting:'Karşı cihazın katılması bekleniyor',awaiting_consent:'Ekranı paylaşan kişinin onayı bekleniyor',negotiating:'Şifreli görüntü bağlantısı kuruluyor',active:'Oturum yetkisi açık',ended:'Deneme sona erdi'};
export function Pilot(){
 const [config,setConfig]=useState<Config>();const [url,setUrl]=useState('');const [invite,setInvite]=useState('');const [name,setName]=useState('Bu bilgisayar');
 ...
 const [videoState,setVideoState]=useState('Görüntü bekleniyor');const [received,setReceived]=useState(false);const [copied,setCopied]=useState(false);const [needsPlay,setNeedsPlay]=useState(false);
 ...
 function closeVideo(){generation.current++;peer.current?.close();peer.current=undefined;negotiating.current=false;media.current?.getTracks().forEach(t=>t.stop());media.current=undefined;if(video.current)video.current.srcObject=null;setReceived(false);setNeedsPlay(false)}
 ...
 useEffect(()=>{
  if(!native)return;
  void invoke<Config>('pilot_config').then(c=>{setConfig(c);setUrl(c.serverUrl);setName(c.platform==='macos'?'Mac cihazı':'Windows cihazı')}).catch(e=>setError(String(e)));
 ...
 async function viewerOffer(offer:RTCSessionDescriptionInit){
  ...
  try{
   ...
   pc.ontrack=e=>{if(current!==generation.current)return;media.current=e.streams[0]??new MediaStream([e.track]);if(video.current){video.current.srcObject=media.current;void video.current.play().catch(()=>{setNeedsPlay(true);setVideoState('Görüntüyü aç düğmesine basın.')})}};
 ...
 async function create(){setError('');setBusy(true);setCopied(false);try{const v=await invoke<{room:Room;invite:string;serverUrl:string}>('pilot_create',{serverUrl:url});active.current=true;lastLease.current=0;setRoom(v.room);setOwnInvite(v.invite);setUrl(v.serverUrl)}catch(e){setError(String(e))}finally{setBusy(false)}}
 ...
 return <>
  <div className="eyebrow">İKİ CİHAZ · DAVETLİ DENEME · PİLOT 2</div><h1>Birlikte bağlanalım.</h1><p className="lead">Bağlanılacak bilgisayarda davet oluşturun. İzleyecek bilgisayarda bu daveti girin. Paylaşım, ekranı açan kişinin kabulüyle başlar.</p>
  ...
  {!room?<div className="two-columns pilot-entry">
   <section className="small-card"><div className="section-title"><Monitor size={20}/><h3>Bu bilgisayarın ekranını paylaş</h3></div><p className="small-description">Bu bilgisayara bağlanılacaksa burada davet oluşturun. 30 dakika geçerli daveti karşı tarafa iletin; gelen isteği ayrıca kabul edin.</p><label className="pilot-field">Sunucu adresi<input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://…trycloudflare.com" autoCapitalize="none" spellCheck={false}/></label><button className="primary" disabled={!config?.canHost||busy||!url.trim()} onClick={create}><Link size={16}/>Davet oluştur</button>{!config?.canHost&&<p className="field-hint">Ekran paylaşımı kurulu Windows veya macOS uygulamasında kullanılabilir.</p>}{config?.platform==='windows'&&<p className="field-hint">Windows pilotu: ilk monitör, en fazla 1280×720. Kilitli ekran ve uzak klavye/fare kontrolü desteklenmez.</p>}</section>
   ...
   {room.role==='host'&&room.state==='waiting'&&<><p className="small-description">Karşı bilgisayarda “Davet ile ekranı izle” bölümüne bu sunucu adresini ve daveti girin. Davet, ekranı sizin kabulünüz olmadan açmaz.</p><label className="pilot-field">Sunucu adresi<input readOnly value={url}/></label><label className="pilot-field">Tek kullanımlık davet<input readOnly value={ownInvite} onFocus={e=>e.target.select()}/></label><button className="secondary" onClick={copy}>{copied?<Check size={15}/>:<Copy size={15}/>}Daveti kopyala</button></>}
   ...
   {room.role==='host'&&['negotiating','active'].includes(room.state)&&<div className="consent"><h2>Ekran paylaşımına izin verdiniz.</h2><p>{room.state==='active'?'Paylaşım yetkisi doğrulandı; görüntü akışı hazırlanıyor.':'Görüntü bağlantısı kuruluyor.'} Paylaşımı bu penceredeki Denemeyi bitir düğmesiyle veya uygulamayı kapatarak durdurabilirsiniz. Klavye ve fare kontrolü verilmez.</p></div>}
   {room.role==='viewer'&&room.state!=='ended'&&<><div className="viewer"><video ref={video} autoPlay muted playsInline controls onPlaying={()=>{setReceived(true);setNeedsPlay(false);setVideoState('Canlı görüntü')}}/>{!received&&<div className="viewer-placeholder">{needsPlay?<button className="primary" onClick={()=>void video.current?.play().catch(()=>setError('Görüntü oynatılamadı. Oturumu yeniden başlatın.'))}>Görüntüyü aç</button>:<LoaderCircle size={25} className="spin"/>}<span>{room.state==='awaiting_consent'?'Karşı bilgisayardaki kullanıcının onayı bekleniyor':videoState}</span></div>}</div><p className="small-description">{videoState} · Görüntü kaydedilmez. Pilot yalnızca izleme destekler.</p></>}
